...
final class HublSpellcheckingStrategy extends SpellcheckingStrategy {

  @Override
  public @NotNull Tokenizer<?> getTokenizer(PsiElement element) {
    if (element instanceof PsiComment) {
      return new HublCommentTokenizer();
    }

    if (element instanceof HublProperty) {
      return new HublPropertyTokenizer();
    }

    return EMPTY_TOKENIZER;
  }

  private static class HublCommentTokenizer extends Tokenizer<PsiComment> {
  ...
  private static class HublPropertyTokenizer extends Tokenizer<HublProperty> {

    public void tokenize(@NotNull HublProperty element, @NotNull TokenConsumer consumer) {
      //Spell check the keys and values of properties with different splitters
      final ASTNode key = element.getNode().findChildByType(HublTypes.KEY);
      ...
      final ASTNode value = element.getNode().findChildByType(HublTypes.VALUE);
